...
async function getDashboardAnalytics(req, res) {
    try {
        ...
        const patents = await dbStore_1.dbStore.patents.find();
        const leads = await dbStore_1.dbStore.interestRequests.find();
        const meetings = await dbStore_1.dbStore.meetingRequests.find();
        const logs = await dbStore_1.dbStore.auditLogs.find();
        const offers = await dbStore_1.dbStore.offers.find();
        const transactions = await dbStore_1.dbStore.transactions.find();
        // 1. ADMIN ANALYTICS
        if (role === 'admin') {
            ...
            for (const p of patents) {
                ...
                if (analysis && analysis.industryClassification) {
                    analysis.industryClassification.forEach((ind) => {
            ...
            for (const t of transactions) {
                ...
                if (t.status === 'escrow_funded') {
                    const unreleasedMilestones = t.milestones.filter((m) => m.status === 'pending');
                    const tActiveEscrow = unreleasedMilestones.reduce((sum, m) => sum + m.amount, 0);
