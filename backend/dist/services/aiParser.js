...
exports.analyzePatentWithAI = analyzePatentWithAI;
exports.queryGeminiLLM = queryGeminiLLM;
// Local Fallback Heuristics in case LLM is unreachable
function getLocalFallback(title, abstract) {
    ...
    // Generic Fallback
    let score = 65;
    let industries = ['General Technology'];
    let keywords = ['innovation', 'system', 'method'];
...
async function analyzePatentWithAI(title, abstract) {
...
IMPORTANT: Return ONLY the raw JSON string starting with "{" and ending with "}". Do NOT wrap the JSON inside markdown code blocks (do not use \`\`\`json ... \`\`\`). Do NOT include any introductory or wrap-up text.`;
    ...
    try {
        ...
        if (!parsed.potentialBuyers)
            parsed.potentialBuyers = parsed.commercialApplications?.potentialIndustries || [];
        ...
        if (!parsed.filingYear)
            parsed.filingYear = 2022;
...
async function queryGeminiLLM(prompt, apiKey) {
