...
export async function checkNDAStatus(req: Request, res: Response) {
  try {
    const authReq = req as IAuthRequest;
    const { id: patentId } = req.params;
    const buyerId = authReq.user!.id;

    const signature = await dbStore.ndaSignatures.findOne({ patentId, buyerId });
...
export async function signNDA(req: Request, res: Response) {
  try {
    const authReq = req as IAuthRequest;
    const { id: patentId } = req.params;
    ...
    const existing = await dbStore.ndaSignatures.findOne({ patentId, buyerId });
    if (existing) {
    ...
    const signature = await dbStore.ndaSignatures.create({
      patentId,
      buyerId,
      fullName,
...
export async function downloadNDADocx(req: Request, res: Response) {
  try {
    const authReq = req as IAuthRequest;
    const { id: patentId } = req.params;
    ...
    let signature = null;
    if (userRole === 'buyer') {
      signature = await dbStore.ndaSignatures.findOne({ patentId, buyerId: userId });
    } else if (userRole === 'owner') {
      // Inventors can download NDAs signed by buyers for their patents
      const patent = await dbStore.patents.findById(patentId);
      if (patent && String(patent.ownerId) === String(userId)) {
        // Find any signature for this patent (or retrieve from query buyerId if specified)
        const buyerId = req.query.buyerId as string;
        if (buyerId) {
          signature = await dbStore.ndaSignatures.findOne({ patentId, buyerId });
        } else {
          // just get the most recent signature
          const sigs = await dbStore.ndaSignatures.find({ patentId });
    ...
    const owner = await dbStore.users.findById(patent.ownerId);
    const buyer = await dbStore.users.findById(signature.buyerId);
                  ...
                  new TableCell({
                    ...
                    children: [
                      new Paragraph({
                        ...
                        children: [new TextRun({ text: "DISCLOSING PARTY (INVENTOR)", bold: true, font: "Times New Roman", size: 20, color: "1F3864" })]
                      })
                    ]
                  }),
                  new TableCell({
                    ...
                    children: [
                      new Paragraph({
                        spacing: { before: 60, after: 60 },
                        children: [new TextRun({ text: "RECEIVING PARTY (ACQUIRER)", bold: true, font: "Times New Roman", size: 20, color: "1F3864" })]
                      })
                    ]
                  })
                ]
                    ...
                    children: [
                      ...
                      new Paragraph({
                        ...
                        children: [
                          ...
                          new TextRun({ text: `/${ownerName.replace(/\s+/g, '')}/`, italics: true, bold: true, color: "777777" })
                        ]
                      }),
                      new Paragraph({
                        spacing: { before: 60, after: 60 },
                        children: [new TextRun({ text: "eSign Status: Platform Pre-Authorized", font: "Times New Roman", size: 18, color: "2E7D32" })]
                      })
                    ]
                  }),
                  new TableCell({
                    ...
                    children: [
                      ...
                      new Paragraph({
                        spacing: { before: 60, after: 60 },
                        children: [
                          new TextRun({ text: `Aadhaar ID: ${signature.aadhaarNumber}`, font: "Times New Roman", size: 20 }),
                        ]
                      }),
                      new Paragraph({
                        spacing: { before: 60, after: 60 },
                        children: [
                          new TextRun({ text: `IP Anchor: ${signature.ipAddress}`, font: "Times New Roman", size: 20 }),
                        ]
                      }),
                      new Paragraph({
                        spacing: { before: 60, after: 60 },
                        children: [
                          new TextRun({ text: `Timestamp: ${dateStr} ${timeStr}`, font: "Times New Roman", size: 20 }),
                        ]
                      }),
                      new Paragraph({
                        spacing: { before: 60, after: 120 },
                        children: [new TextRun({ text: "eSign Status: Verified via UIDAI OTP eSign Gateway", font: "Times New Roman", size: 18, color: "2E7D32", bold: true })]
                      })
                    ]
                  })
                ]
...
export async function getSignedNDAs(req: Request, res: Response) {
  try {
    const authReq = req as IAuthRequest;
    ...
    let signatures: any[] = [];
    if (userRole === 'buyer') {
      signatures = await dbStore.ndaSignatures.find({ buyerId: userId });
    } else if (userRole === 'owner') {
      const allSigs = await dbStore.ndaSignatures.find();
      for (const sig of allSigs) {
        const patent = await dbStore.patents.findById(sig.patentId);
        if (patent && String(patent.ownerId) === userId) {
          signatures.push(sig);
        }
      }
    } else if (userRole === 'admin') {
      signatures = await dbStore.ndaSignatures.find();
    }

    // Enrich with Patent Title and Counter-Party details
    const enriched = [];
    for (const sig of signatures) {
      const p = await dbStore.patents.findById(sig.patentId);
      const buyer = await dbStore.users.findById(sig.buyerId);
