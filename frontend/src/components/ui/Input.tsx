...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type = 'text', label, error, labelClassName, ...props }, ref) => {
    return (
      <div className="w-full">
        {label && (
          <label className={cn('type-label block text-zinc-500 mb-1.5', labelClassName)}>
            {label}
          </label>
        )}
        <input
        ...
        />
        {error && (
          <span className="block mt-1.5 text-sm text-red-600 font-medium">{error}</span>
        )}
      </div>
    );
  }
